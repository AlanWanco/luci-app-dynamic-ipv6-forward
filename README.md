...
- 在 LuCI 中管理内网设备和端口转发；
- 默认提供“本路由器”设备选项，自动使用 LAN 接口当前全局 IPv6；
- 复用 LuCI 主机提示，将已发现设备的 MAC 地址显示为可选下拉项；
- 按 MAC/NDP 自动发现设备当前全局 IPv6；
- 支持备用 64 位 IID 和手动 IPv6 地址；
- 支持 TCP、UDP、单端口和端口范围；
- 支持公网端口与内部监听端口分离；
- 支持按来源 IPv6/CIDR 限制访问；
- 状态页面显示设备发现结果、实际防火墙规则和 TCP 的 IPv6 监听检查；
...
1. 在“设备”中添加内网设备，在 MAC 地址字段中从已发现设备下拉选择，或手动输入 MAC；
2. 若输入的 MAC 有 DHCP/DNS 主机名，设备名称会自动填入；已有手动名称不会被覆盖；
3. 设备所在接口填写 Linux 接口名，例如 `br-lan` 或 `eth0.2`；
4. 设备地址暂时无法从 NDP 发现时，再配置备用 IID；
5. 每个 TCP/UDP 端口分别建立规则；
6. 公网端口填写外部访问端口，内部端口填写设备实际监听端口；
7. 保存应用后，在状态表中确认设备 IPv6 和规则状态为“已生效”；对于 TCP 规则，还要确认“IPv6 监听检查”显示可连接。

服务只绑定 `0.0.0.0:端口` 时，通常只监听 IPv4，不能接收本插件转发的 IPv6 流量。服务应监听 `[::]:端口` 或明确启用双栈。UDP 没有通用的连接握手，状态页面不会对 UDP 做可靠的开放性判断，需要从公网实际测试。
...
- 默认的“本路由器”不需要填写 MAC 或备用 IID，会自动解析所选 LAN 接口上的当前全局 IPv6；
- **名称**：仅用于显示；
...
- 状态页的 TCP 检查由路由器从 LAN 侧通过目标 IPv6 发起，检查失败通常表示服务没有监听 IPv6、设备防火墙拦截或设备暂时离线；
- UDP 状态显示为“未检查”，不能据此判断 UDP 端口关闭。
...
- LAN 必须有可路由的全局 IPv6；
- 运行时需要 `ip-full`；
- 状态页的 TCP 监听检查使用路由器上的 `nc`；如果系统没有该命令，会显示“无法检查”，不影响端口转发；
